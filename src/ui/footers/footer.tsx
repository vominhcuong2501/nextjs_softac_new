    ...
    return (
        <>
            <div className='border-b border-gray-41 py-5 lg:py-10 leading-1-5'>
                <div className='container'>
                    <div className='grid grid-cols-1 sm:grid-cols-2 lg:flex lg:justify-between gap-5'>
                        <Motion effect='fadeInUp' className='md:max-w-[275px] w-full' transition={{ delay: 0 }} >
                        ...
                        {footer_lisks.map((item, i) => (
                            <Motion effect='fadeInUp' key={i} transition={{ delay: 0.2 + i * 0.3 }}>
                        ...
                        <Motion effect='fadeInUp' className='md:max-w-[334px] w-full' transition={{ delay: 0.2 + footer_lisks.length * 0.3 }} >
                            ...
                            <form onSubmit={(e) => e.preventDefault()} className='relative mt-4 lg:mt-7'>
                                <input type='text' placeholder='Business email adress' className='h-[50px] lg:h-[60px] pl-[55px] pr-20 bg-gray-8 rounded-[40px] text-14 focus:outline-none w-full' />
                                    ...
                                    {social_links?.map(social => {
                                        return (
                                            <Link href={social.link} target='_blank' title={social.name} key={social.name} >
                                                <i className={`text-gray-49 hover:text-purple-2 transition-all duration-200 ${social.icon}`}></i>
                                            </Link>
                                        )
                                    })}
                                </div>
            ...
            <div className='py-5 flex items-center justify-center gap-1 text-15 font-medium text-gray-1 font-urban'>
                <p>{text_copyright} <a href='#' title='Theme pure' target='_blank' className='text-black'>{text_theme_pure}</a> - {new Date().getFullYear()}</p>
            </div>
        </>
    )
}
